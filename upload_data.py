...
# //-------------------------------------------------------------------------//
    ...
    def upload():
         ...
         for inp in data_list[:]:
            barcode = inp['inp_barcode'].get()
            name = inp['inp_name'].get()
            cost_price = float(inp['inp_cost_price'].get())
            sell_price = float(inp['inp_price'].get())
            amount = int(inp['inp_amount'].get())

            print(f"{barcode}, {name}, {cost_price}, {sell_price}, {amount}")
            ...
            try:
                ...
                gui.destroy()
                 ...
                 CTkMessagebox(title="Error",message=f"Error: {err}",icon="cancel",option_1="Ok")
                    ...
                    CTkMessagebox(title="Error",message=f"Error: {err}",icon="cancel",option_1="Ok")
            remove_track(inp,inp["btn_remove"].master)
        
            
      

    btn_upload = CTkButton(gui,font=("Arial",16),command=upload,width=60,height=30,corner_radius=20,text="Upload",text_color="white",fg_color="#38f388",hover_color="#6be59e")
